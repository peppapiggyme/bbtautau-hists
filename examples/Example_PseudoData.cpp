...
static void test_pseudodata_helper(const std::string& filename,
    double& obs_q0, double& obs_sig, int& status_con, int& status_uncon, int& sign);
...
void test_pseudodata(const std::vector<std::string>& filenames, const std::string& outfile)
{
    ...
    for (const auto& filename : filenames)
    {
        ...
        test_pseudodata_helper(filename, obs_q0, obs_sig, status_con, status_uncon, sign);
    ...
    double& obs_q0, double& obs_sig, int& status_con, int& status_uncon, int& sign)
{
    ...
    wst->GetRooWorkspace()->loadSnapshot("conditionalNuis_0");
