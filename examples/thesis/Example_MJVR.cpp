...
void thesis_mjvr(const std::string& filename)
{
...
#ifndef RUN_2BSS
    rs->add("1tag2pjet_0ptv_LL_OS",     "1-b-tagged OS MJ VR",         eRegionType::SR);
#else
    rs->add("2tag2pjet_0ptv_LL_SS",     "2-b-tagged SS MJ VR",         eRegionType::SR);
#endif
    ...
    for (VariableInfo* v : *(vs->content()))
    {
        ...
        ps->add("Fake",       "Multi-jet",          eProcessType::BKG,      eProcess::P1,           "Multi-jet",                        bbtt_kPINK);
#ifndef RUN_2BSS
...
#endif
        ps->add("Zttbb",      "Z#tau#tau + bb",     eProcessType::BKG,      eProcess::P3,           "Z #rightarrow #tau#tau + hf",      bbtt_kBLUE_L);
        ps->add("Zttbc",      "Z#tau#tau + bc",     eProcessType::BKG,      eProcess::P3,           "Z #rightarrow #tau#tau + hf",      bbtt_kBLUE_L);
        ps->add("Zttcc",      "Z#tau#tau + cc",     eProcessType::BKG,      eProcess::P3,           "Z #rightarrow #tau#tau + hf",      bbtt_kBLUE_L);
#ifndef RUN_2BSS
        ps->add("ttbar",      "t#bar{t}",           eProcessType::BKG,      eProcess::P6,           "t#bar{t}",                         bbtt_kYELLOW);
        ps->add("ttbarSFTF",  "t#bar{t} TF",        eProcessType::BKG,      eProcess::P6,           "t#bar{t}",                         bbtt_kYELLOW);
        ps->add("ttbarSFFT",  "t#bar{t} FT",        eProcessType::BKG,      eProcess::P6,           "t#bar{t}",                         bbtt_kYELLOW);
        ps->add("ttbarSFFF",  "t#bar{t} FF",        eProcessType::BKG,      eProcess::P6,           "t#bar{t}",                         bbtt_kYELLOW);
#else
        ...
        ds->output_path = "/scratchfs/atlas/bowenzhang/bbtautau-hists/output/forThesis/MJVR/";
        if (ds->check(c))
        {
            ds->manipulate(c);
