...
void thesis_sr_ws_bdt_klam(const std::string& filename, bool prefit)
{
    ...
    for (VariableInfo* v : *(vs->content()))
    {
        ...
        // ps->add("hhttbb",     "ggF HH SM",          eProcessType::SIG,      eProcess::P16,          "ggF HH SM",                        bbtt_kSIG_RED);
        // ps->add("hhttbbKLRW0p0", "ggF HH #kappa_{#lambda}=0", eProcessType::SIG, eProcess::P101, "ggF HH #kappa_{#lambda}=0",           bbtt_kSIG_ORANGE);
        ps->add("hhttbbKLRW2p0", "ggF HH #kappa_{#lambda}=2", eProcessType::SIG, eProcess::P102, "ggF HH #kappa_{#lambda}=2",           bbtt_kSIG_RED);
        ps->add("hhttbbKLRW5p0", "ggF HH #kappa_{#lambda}=5", eProcessType::SIG, eProcess::P103, "ggF HH #kappa_{#lambda}=5",           bbtt_kSIG_ORANGE);

        Config* c = new Config(b, ps, rs, vs);
        c->load(filename, "Region_BMin0_incJet1_distSMBDT_J2_Y2015_DLLOS_T2_SpcTauHH_L0");
        // info->parameter = "BDT_Klam_1";
        info->parameter = "BDT_Klam_2";
