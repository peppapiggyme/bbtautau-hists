...
static void thesis_sr_ws_lastbin_helper(const std::string& region, const std::string& filename, const std::string& mass, 
...
{
    thesis_sr_ws_lastbin_helper(region, filename, mass, "dRBB", "#Delta R(b, b)", "Region_BMin0_incJet1_distdRBB_J2_Y2015_DLLOS_T2_SpcTauHH_L0");
    thesis_sr_ws_lastbin_helper(region, filename, mass, "dRTauTau", "#Delta R(#tau, #tau)", "Region_BMin0_incJet1_distdRTauTau_J2_Y2015_DLLOS_T2_SpcTauHH_L0");
    thesis_sr_ws_lastbin_helper(region, filename, mass, "mBB", "m_{bb} [GeV]", "Region_BMin0_incJet1_distmBB_J2_Y2015_DLLOS_T2_SpcTauHH_L0");
    thesis_sr_ws_lastbin_helper(region, filename, mass, "mHH", "m_{HH} [GeV]", "Region_BMin0_incJet1_distmHH_J2_Y2015_DLLOS_T2_SpcTauHH_L0");
    thesis_sr_ws_lastbin_helper(region, filename, mass, "mMMC", "m_{#tau#tau}^{MMC} [GeV]", "Region_BMin0_incJet1_distmMMC_J2_Y2015_DLLOS_T2_SpcTauHH_L0");
}

void thesis_sr_ws_lastbin_helper(const std::string& region, const std::string& filename, const std::string& mass, 
    const std::string& varname, const std::string& varname_tex, const std::string& folder)
{
    ...
    rs->add("2tag2pjet_0ptv_LL_OS",     region,         eRegionType::SR);
    ...
    for (VariableInfo* v : *(vs->content()))
    {
        ...
        info->parameter = "Input_PNN"+mass;

        c->updateHistogramPtr(rs->content()->back(), v);
        DrawStackTool* ds = new DrawStackTool_WS(info);
        ds->output_path = "/scratchfs/atlas/bowenzhang/bbtautau-hists/output/forThesis/LastBin_WS_Post/";
        if (ds->check(c))
        {
            ds->manipulate(c);
            ds->paint(c);
