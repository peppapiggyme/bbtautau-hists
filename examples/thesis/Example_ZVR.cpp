...
// tune this to plot 1tag or 2tag ...
// #define DO_ONE_TAG 1
...
void thesis_zvr(const std::string& filename)
{
    ...
    rs->add("1tag2pjet_0ptv_LL_OS",     "1-b-tagged ZVR",         eRegionType::SR);

    vector<double> vMMC{65, 67.5, 70, 72.5, 75, 77.5, 80, 82.5, 85, 87.5, 90, 92.5, 95, 97.5, 
                        100, 102.5, 105, 107.5, 110, 112.5, 115, 117.5, 120,};

    Variables* vs = new Variables();
    vs->add("mBB",                  "m_{bb} [GeV]",                           10);
    vs->add("mMMC",                 "m_{#tau#tau}^{MMC} [GeV]",               10, &vMMC[0], vMMC.size()-1);
    ...
    for (VariableInfo* v : *(vs->content()))
    {
        ...
        ps->add("Zttbb",      "Z#tau#tau + bb",     eProcessType::BKG,      eProcess::P2,           "Z #rightarrow #tau#tau + hf",      bbtt_kBLUE_L);
        ps->add("Zttbc",      "Z#tau#tau + bc",     eProcessType::BKG,      eProcess::P2,           "Z #rightarrow #tau#tau + hf",      bbtt_kBLUE_L);
        ps->add("Zttcc",      "Z#tau#tau + cc",     eProcessType::BKG,      eProcess::P2,           "Z #rightarrow #tau#tau + hf",      bbtt_kBLUE_L);
        ps->add("Zttbl",      "Z#tau#tau + bl",     eProcessType::BKG,      eProcess::P1,           "Z #rightarrow #tau#tau + lf",      bbtt_kBLUE_D);
        ps->add("Zttcl",      "Z#tau#tau + cl",     eProcessType::BKG,      eProcess::P1,           "Z #rightarrow #tau#tau + lf",      bbtt_kBLUE_D);
        ps->add("Zttl",       "Z#tau#tau + l",      eProcessType::BKG,      eProcess::P1,           "Z #rightarrow #tau#tau + lf",      bbtt_kBLUE_D);
        ...
        ds->output_path = "/scratchfs/atlas/bowenzhang/bbtautau-hists/output/forThesis/ZVR/";
        if (ds->check(c))
        {
            ds->manipulate(c);
...
void thesis_zvr_2tag(const std::string& filename)
{
    ...
    Regions* rs = new Regions();
    rs->add("2tag2pjet_0ptv_LL_OS",     "2-b-tagged ZVR",         eRegionType::SR);
    ...
    for (VariableInfo* v : *(vs->content()))
    {
        ...
        ps->add("Zttbl",      "Z#tau#tau + bl",     eProcessType::BKG,      eProcess::P2,           "Z #rightarrow #tau#tau + lf",      bbtt_kBLUE_D);
        ps->add("Zttcl",      "Z#tau#tau + cl",     eProcessType::BKG,      eProcess::P2,           "Z #rightarrow #tau#tau + lf",      bbtt_kBLUE_D);
        ps->add("Zttl",       "Z#tau#tau + l",      eProcessType::BKG,      eProcess::P2,           "Z #rightarrow #tau#tau + lf",      bbtt_kBLUE_D);
        ps->add("ttbar",      "t#bar{t}",           eProcessType::BKG,      eProcess::P4,           "True-#tau_{had} t#bar{t}",         bbtt_kGOLDEN);
        ps->add("ttbarSFTF",  "t#bar{t} TF",        eProcessType::BKG,      eProcess::P6,           "Fake-#tau_{had} t#bar{t}",         bbtt_kORANGE);
        ps->add("ttbarSFFT",  "t#bar{t} FT",        eProcessType::BKG,      eProcess::P6,           "Fake-#tau_{had} t#bar{t}",         bbtt_kORANGE);
        ps->add("ttbarSFFF",  "t#bar{t} FF",        eProcessType::BKG,      eProcess::P6,           "Fake-#tau_{had} t#bar{t}",         bbtt_kORANGE);
